...
pub fn collect_db_dirs<'a, T: AsRef<Path>>(dir: T, projects: impl IntoIterator<Item=&'a str>) -> io::Result<Vec<PathBuf>> {
            ...
            Ok(entries) => {
                entries.into_iter().filter_map(|entry| {
                    match entry {
                        Ok(entry) => {
                            let path = entry.path();
                            if path.is_dir() && path.file_name().unwrap().to_str().unwrap().ends_with("000") {
                                Some(Ok(path))
                            } else {
                                None
                            }
