...
async fn test_poinsp_position_math_analysis() -> Result<()> {
    ...
    println!("   East-West (X): {:.2} mm ({} West)", expected_world_pos.x.abs(), 
             if expected_world_pos.x < 0.0 { "" } else { "East" });
    println!("   North-South (Y): {:.2} mm ({} North)", expected_world_pos.y.abs(),
             if expected_world_pos.y > 0.0 { "" } else { "South" });
    println!("   Up-Down (Z): {:.2} mm ({} Down)", expected_world_pos.z.abs(),
             if expected_world_pos.z < 0.0 { "" } else { "Up" });
    
    // 计算位置的方向角度（用于验证坐标系）
    let horizontal_dist = (expected_world_pos.x * expected_world_pos.x + 
                          expected_world_pos.y * expected_world_pos.y).sqrt();
    let azimuth = expected_world_pos.y.atan2(expected_world_pos.x).to_degrees();
    ...
    // 验证位置合理性
    assert!(expected_world_pos.x < -5000.0, "Should be significantly West");
    assert!(expected_world_pos.y > 1000.0, "Should be significantly North");
    assert!(expected_world_pos.z < -2000.0, "Should be significantly Down");
...
async fn test_spine_coordinate_system_analysis() -> Result<()> {
    ...
    // 基于前面测试的方向数据
    let spine_z = glam::DVec3::new(-0.0007869044836398384, 0.9998344368711255, -0.01817909865569267);
    let spine_y = glam::DVec3::new(-1.4307578617685256e-5, 0.01817909302541243, 0.9998347465316791);
    let spine_x = glam::DVec3::new(-0.9999996902882655, -0.0007870345438278944, 5.421010862427522e-20);
    ...
    fn analyze_axis(name: &str, axis: glam::DVec3) {
        ...
        if abs_x > 0.9 {
            println!("   Primary direction: {}", if axis.x > 0.0 { "East" } else { "West" });
        } else if abs_y > 0.9 {
            println!("   Primary direction: {}", if axis.y > 0.0 { "North" } else { "South" });
        } else if abs_z > 0.9 {
            println!("   Primary direction: {}", if axis.z > 0.0 { "Up" } else { "Down" });
        } else {
    ...
    if verticality > 0.99 {
        ...
        println!("   Type: Inclined component (verticality: {:.1}%)", verticality * 100.0);
    }
