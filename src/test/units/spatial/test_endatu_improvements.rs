...
use crate::transform::strategies::{EndatuError, EndatuValidator, EndAtuStrategy, EndAtuZdisHandler, EndatuResult, TransformStrategy};
...
async fn test_error_code_mapping() -> Result<()> {
    ...
    let test_cases = vec![
        ...
        (EndatuError::GeometryCalculationError("test".to_string()), 252),
    ];
...
async fn test_caching_mechanism() -> Result<()> {
    use crate::transform::strategies::{get_cached_endatu_index, get_cache_stats};
    use crate::RefnoEnum;
    ...
    println!("   ✅ 缓存机制测试通过，命中率: {:.2}%", stats.hit_rate() * 100.0);
    Ok(())
}
...
async fn test_attribute_priority() -> Result<()> {
    ...
    // 测试用例 1: ZDIS 优先级最高
    {
        ...
        att.insert("ZDIS".to_string(), AttrVal::DoubleType(100.0).into());
        att.insert("OPDI".to_string(), AttrVal::Vec3Type([1.0, 0.0, 0.0]).into());
        att.insert("YDIR".to_string(), AttrVal::Vec3Type([0.0, 1.0, 0.0]).into());
        ...
        let result: anyhow::Result<Option<glam::DMat4>> = strategy.get_local_transform(refno, parent_refno, &att, &parent_att).await;
...
async fn test_zdis_processing() -> Result<()> {
    ...
    // 测试用例 1: 无 ZDIS 属性
    {
        ...
        let result: EndatuResult<bool> = EndAtuZdisHandler::handle_endatu_zdis(
