...
async fn debug_poinsp_17496_266220_position_calculation() -> Result<()> {
    ...
    // 构建变换矩阵
    let gensec_mat = DMat4::from_scale_rotation_translation(
        gensec_scale, 
        gensec_rotation, 
        gensec_translation
    );
...
async fn debug_gensec_rotation_effect() -> Result<()> {
    ...
    let mat1 = DMat4::from_scale_rotation_translation(
        glam::DVec3::ONE, no_rotation, gensec_translation
    );
    ...
    let mat2 = DMat4::from_scale_rotation_translation(
        glam::DVec3::ONE, rot_z, gensec_translation
    );
    ...
    let mat3 = DMat4::from_scale_rotation_translation(
        glam::DVec3::ONE, rot_45, gensec_translation
    );
