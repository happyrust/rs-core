...
async fn test_spine_virtual_node_orientation() -> Result<()> {
    ...
    assert!(dot_product > 0.999, "虚拟节点方向应该与get_world_mat4方向一致");
...
async fn test_poinsp_with_virtual_node_system() -> Result<()> {
    ...
    // 获取POINSP的局部位置
    let poinsp_local_pos = att.get_position().expect("POINSP should have POS").as_dvec3();
    ...
    assert!(pos_diff.length() < 0.01, "虚拟节点系统计算应该与实际结果一致");
