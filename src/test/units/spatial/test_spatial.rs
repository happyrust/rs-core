use crate::rs_surreal::spatial::{get_world_mat4, get_world_mat4_old};
use crate::*;
use anyhow::Result;
...
async fn debug_specific_refnos() -> Result<()> {
    ...
    // 手动调用策略计算
    match strategy
        .get_local_transform(refno, owner, &att, &parent_att)
        .await
    {
    ...
    // 使用旧函数（作为对比基准）
    #[allow(deprecated)]
    let old_world_matrix = get_world_mat4_old(refno, false).await?;

    // 使用新的策略系统函数
    let new_world_matrix = get_world_mat4(refno, false).await?;
        ...
        (Some(old), Some(new)) => {
            ...
            if let Some(world_matrix) = new_world_matrix {
                ...
                if noun == "FITT" {
                    ...
                    // 分析局部变换结果
                    match get_world_mat4(refno, true).await {
                        Ok(Some(local_matrix)) => {
                            let local_pos: DVec3 = local_matrix.project_point3(glam::DVec3::ZERO);
...
async fn test_generic_spatial_cases() -> Result<()> {
    ...
    let test_cases: Vec<SpatialTestCase> =
    ...
    for case in &test_cases {
        ...
        if let Some(world_matrix) = get_world_mat4(target_refno, false).await? {
            ...
            if diff.length() < 1.0 {
                println!(
                    "✅ Position OK - Expected: {:?}, Got: {:?}, Diff: {:.4}",
                    expected_pos,
                    calculated_pos,
                    diff.length()
                );
                ...
                let msg = format!(
                    "❌ Position Mismatch for {}: Expected {:?}, Got {:?}, Diff {:.4}",
                    case.refno,
                    expected_pos,
                    calculated_pos,
                    diff.length()
            ...
            if let Some((expected_y, expected_z)) = parse_wori(&case.wori_str) {
                ...
                if y_dot > 0.999 && z_dot > 0.999 {
                    println!(
                        "✅ Orientation OK - Y_dot: {:.6}, Z_dot: {:.6}",
                        y_dot, z_dot
                    );
                } else {
                    let msg = format!(
                        "⚠️  Orientation Mismatch for {}: Y_dot={:.6}, Z_dot={:.6}",
    ...
    let problem_refnos = vec!["17496/266220", "25688/7960"];
    ...
    for refno_str in problem_refnos {
        ...
        if let Ok(att) = get_named_attmap(refno).await {
            ...
            // 检查策略使用
            let strategy =
                crate::transform::strategies::TransformStrategyFactory::get_strategy(noun);
            println!("🎯 使用策略: {:?}", std::any::type_name_of_val(&strategy));
...
async fn debug_fitt_zdis_issue() -> Result<()> {
    ...
    if let Some(world_matrix) = transform::get_world_mat4(refno).await? {
        ...
        println!(
            "   期望结果: {:?}",
            glam::DVec3::new(59375.0, 21200.0, -7350.0)
        );
