...
async fn test_poinsp_17496_266220_local_transform() -> Result<()> {
    ...
    let spine_world_mat = get_world_mat4(owner_refno, false)
        .await?
    ...
    println!(
        "📋 结论: POINSP的POS使用特殊坐标系(Y=沿SPINE距离,X/Z=横向偏移)，需要通过calculate_poinsp_spine_transform解释"
    );
...
async fn test_spine_local_transform_analysis() -> Result<()> {
    ...
    let spine_world_mat = get_world_mat4(spine_refno, false)
        .await?
        .expect("SPINE should have world matrix");
    let gensec_world_mat = get_world_mat4(owner_refno, false)
        .await?
        .expect("GENSEC should have world matrix");
