...
pub fn create_test_attmap_with_bang(bang_value: f32) -> NamedAttrMap {
    ...
    att.insert("BANG".to_string(), AttrVal::DoubleType(bang_value as f64).into());
    att.insert("NOUN".to_string(), AttrVal::StringType("TEST".to_string()).into());
    att
}
...
pub fn create_test_attmap_with_zdis(zdis_value: f32, pkdi_value: f32) -> NamedAttrMap {
    let mut att = NamedAttrMap::new("TEST");
    att.insert("ZDIS".to_string(), AttrVal::DoubleType(zdis_value as f64).into());
    att.insert("PKDI".to_string(), AttrVal::DoubleType(pkdi_value as f64).into());
    att.insert("NOUN".to_string(), AttrVal::StringType("TEST".to_string()).into());
    att
}
...
pub fn create_complete_test_attmap() -> NamedAttrMap {
    ...
    att.insert("POSL".to_string(), AttrVal::StringType("".to_string()).into());
...
pub fn create_test_attmap_with_attributes() -> NamedAttrMap {
    ...
    // 基本属性
    att.insert("NOUN".to_string(), AttrVal::StringType("ENDATU".to_string()).into());
    let pos_f64 = [0.0, 0.0, 0.0];
    att.insert("NPOS".to_string(), AttrVal::Vec3Type(pos_f64).into());
    
    // 设置所有者
    att.insert("OWNER".to_string(), AttrVal::StringType("test_parent".to_string()).into());
