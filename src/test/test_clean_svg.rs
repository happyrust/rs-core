...
async fn test_clean_svg_without_labels() {
    ...
    assert!(svg_content.contains("path-line"), "Should contain line paths");
    assert!(svg_content.contains("path-arc"), "Should contain arc paths");
    assert!(svg_content.contains("point-poinsp"), "Should contain POINSP points");
    assert!(svg_content.contains("point-curve"), "Should contain CURVE points");

    // 验证不包含长度标签
    assert!(!svg_content.contains("150.0mm"), "Should not contain length labels");
    assert!(!svg_content.contains("Arc:"), "Should not contain arc labels");
    assert!(!svg_content.contains("直线"), "Should not contain Chinese text '直线'");
    assert!(!svg_content.contains("弧线"), "Should not contain Chinese text '弧线'");
...
async fn test_minimal_svg() {
    ...
    svg_generator.add_point("CURVE".to_string(), Vec3::new(400.0, 200.0, 0.0), Some(100.0));
...
async fn test_custom_display_options() {
    ...
    svg_generator.add_point("CURVE".to_string(), Vec3::new(300.0, 100.0, 0.0), Some(75.0));
    ...
    // 验证包含坐标但不包含长度标签
    let svg_content = svg_generator.generate_svg();
    assert!(svg_content.contains("(0,0)"), "Should contain coordinate labels");
    assert!(!svg_content.contains("mm"), "Should not contain length labels");
    assert!(!svg_content.contains("SPINE路径信息"), "Should not contain legend");
