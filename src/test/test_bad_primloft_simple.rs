...
use crate::prim_geo::wire::{process_ploop_vertices, gen_polyline_from_processed_vertices};
    ...
    if let CateProfileParam::SPRO(spro) = &profile {
        ...
        for i in 0..spro.verts.len() {
            ...
            if i < 4 || spro.frads[i] > 0.0 || spro.frads[(i+1) % spro.verts.len()] > 0.0 {
                println!("  边 {} -> {}: 长度 = {:.2} mm", i, (i+1) % spro.verts.len(), edge_len);
            }
        }
        ...
        for (i, &frad) in spro.frads.iter().enumerate() {
            if frad > 0.0 {
                ...
                if frad * 2.0 > edge1_len {
                    println!("    ⚠️ 警告: FRADIUS 的 2 倍 ({:.2}) 大于前一条边 ({:.2})!", frad * 2.0, edge1_len);
                }
                if frad * 2.0 > edge2_len {
                    println!("    ⚠️ 警告: FRADIUS 的 2 倍 ({:.2}) 大于后一条边 ({:.2})!", frad * 2.0, edge2_len);
                }
            }
        }
            ...
            Ok(processed) => {
                ...
                match gen_polyline_from_processed_vertices(&processed, None) {
                    Ok(polyline) => {
                        ...
                        let wire_verts: Vec<Vec3> = polyline.vertex_data.iter()
                            .map(|v| Vec3::new(v.x as f32, v.y as f32, v.bulge as f32))
                            .collect();
                        let wire_frads: Vec<f32> = polyline.vertex_data.iter()
