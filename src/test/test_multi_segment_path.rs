...
use crate::prim_geo::spine::{SweepPath3D, SegmentPath, Line3D, Arc3D, Spine3D, SpineCurveType};
use glam::{Vec3, DVec3};
...
fn test_single_arc_path() {
    ...
    assert!((actual_length - expected_length).abs() < 0.001, 
            "圆弧长度应该约为 {:.3}，实际为 {:.3}", expected_length, actual_length);
...
fn test_multi_segment_path() {
    ...
    // 验证辅助方法对多段路径返回 None
    assert!(path.as_single_line().is_none(), "多段路径不应该返回单段直线");
    ...
    assert!((actual_total - expected_total).abs() < 0.01,
            "总长度应该约为 {:.3}，实际为 {:.3}", expected_total, actual_total);
    
    println!("  直线1长度: {:.3}", line1_len);
    println!("  圆弧长度: {:.3}", arc_len);
    println!("  直线2长度: {:.3}", line2_len);
    println!("  总长度: {:.3} (预期: {:.3})", actual_total, expected_total);
    println!("✅ 多段混合路径测试通过");
}
...
fn test_path_continuity_check() {
    ...
    assert_eq!(discontinuity_index, Some(0), "不连续点应该在段0和段1之间（返回段0的索引）");
    println!("✅ 不连续路径检测通过");
}
...
fn test_path_geometry_properties() {
    ...
    // 第一段是沿X轴，切线应该是 (1, 0, 0)
    assert!((tangent_start - Vec3::X).length() < 0.01, "起点切线应该沿X轴");
...
fn test_gensec_spine_scenario() {
    ...
    assert!((actual - expected).abs() < 1.0,
            "总长度应该约为 {:.3}，实际为 {:.3}", expected, actual);
...
fn test_path_iteration() {
    ...
    for (i, segment) in path.segments.iter().enumerate() {
        match segment {
            ...
            SegmentPath::Arc(a) => {
                arc_count += 1;
                println!("    段{}: 圆弧, 半径={:.3}, 角度={:.3}°", 
                         i, a.radius, a.angle.to_degrees());
            }
        }
    }
...
fn test_multi_segment_sweep_solid_creation() {
    ...
    assert!(multi_path.as_single_line().is_none(), "多段路径不应该返回单段直线");
    
    // 验证总长度（200 + 150*π/2 + 300 ≈ 735.4mm)
    let expected_length = 200.0 + 150.0 * PI / 2.0 + 300.0;
    assert!((total_length - expected_length).abs() < 1.0,
            "总长度应该约为{:.3}mm, 实际为{:.3}mm", expected_length, total_length);
