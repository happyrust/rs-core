...
fn test_rsvec3_deref_methods() {
    ...
    println!("✓ 直接调用 Vec3 方法: length={}, normalized={:?}", length, normalized);
}
...
fn test_rsvec3_field_access() {
    ...
    println!("✓ 直接访问字段: x={}, y={}, z={}", rs_vec.x, rs_vec.y, rs_vec.z);
}
...
fn test_rsvec3_with_transform() {
    ...
    println!("✓ 变换场景: original={:?}, offset={:?}, result={:?}", pos, offset, scaled_pos);
}
...
fn test_rsvec3_vec3_reference_ops() {
    ...
    println!("✓ 引用运算: 原值保留，result1={:?}, result2={:?}", result1, result2);
}
...
fn test_practical_mixed_usage() {
    ...
    println!("✓ 实际混合场景: direction={:?}, reverse={:?}", direction, reverse_dir);
}
