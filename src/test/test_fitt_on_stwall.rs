use anyhow::Result;
use glam::{DVec3, DMat4, DQuat, dvec3, Vec3};
use std::str::FromStr;
use crate::{RefnoEnum, NamedAttrMap};
...
async fn test_fitt_on_stwall_position_calculation() -> Result<()> {
    ...
    // 执行计算
    let result = calculate_fitt_on_stwall_transform(stwall_refno, fitt_refno, &stwall_att, &fitt_att).await?;
    ...
    if actual_position.length() < 10000.0 { // 简单判断是否缺失了由50000+组成的大坐标
    ...
    println!("位置误差: {:.3}mm", (actual_position - expected_position).length());
...
async fn calculate_fitt_on_stwall_transform(
    ...
    println!("Transform Z axis: {:?}", transform.transform_vector3(DVec3::Z));
    println!("Transform Y axis: {:?}", transform.transform_vector3(DVec3::Y));
