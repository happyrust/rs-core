...
fn test_get_named_attmap() {
    ...
    original_map.insert(
        "TYPE".to_string(),
        NamedAttrValue::StringType("EQUI".to_string()),
    );
    original_map.insert(
        "NAME".to_string(),
        NamedAttrValue::StringType("Test Equipment".to_string()),
    );
    original_map.insert(
        "REFNO".to_string(),
        NamedAttrValue::RefU64Type(crate::RefU64::from_two_nums(1112, 12345)),
    );
    original_map.insert(
        "OWNER".to_string(),
        NamedAttrValue::RefU64Type(crate::RefU64::from_two_nums(1112, 10000)),
    );
    ...
    original_map.insert(
        "POS".to_string(),
        NamedAttrValue::Vec3Type(Vec3::new(100.0, 200.0, 300.0)),
    );
    original_map.insert(
        "DIMS".to_string(),
        NamedAttrValue::F32VecType(vec![10.0, 20.0, 30.0]),
    );
    original_map.insert(
        "TAGS".to_string(),
        NamedAttrValue::StringArrayType(vec!["tag1".to_string(), "tag2".to_string()]),
    );
    original_map.insert(
        "FLAGS".to_string(),
        NamedAttrValue::IntArrayType(vec![1, 2, 3]),
    );
    original_map.insert("ACTIVE".to_string(), NamedAttrValue::BoolType(true));
    original_map.insert(
        "STATES".to_string(),
        NamedAttrValue::BoolArrayType(vec![true, false, true]),
    );
    ...
    println!(
        "Restored map keys: {:?}",
        restored_map.keys().collect::<Vec<_>>()
    );
    ...
    assert!(
        restored_map.contains_key("REFNO"),
        "Should contain REFNO key"
    );
    assert!(
        restored_map.contains_key("OWNER"),
        "Should contain OWNER key"
    );
    assert!(restored_map.contains_key("PGNO"), "Should contain PGNO key");
    assert!(
        restored_map.contains_key("SESNO"),
        "Should contain SESNO key"
    );
