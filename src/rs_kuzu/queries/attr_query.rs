...
pub async fn get_named_attmap_kuzu(refno: RefnoEnum) -> Result<NamedAttrMap> {
    ...
    while let Some(record) = result.next() {
        ...
        if let (Some(name_val), Some(value_val), Some(type_val)) =
            ...
            if let (kuzu::Value::String(name), kuzu::Value::String(value_str), kuzu::Value::String(attr_type)) =
...
#[cfg(feature = "kuzu")]
/// 批量查询属性
pub async fn get_batch_attmaps_kuzu(refnos: &[RefnoEnum]) -> Result<Vec<(RefnoEnum, NamedAttrMap)>> {
    if refnos.is_empty() {
        return Ok(vec![]);
    }
    ...
    while let Some(record) = result.next() {
        if let (Some(refno_val), Some(name_val), Some(value_val), Some(type_val)) =
            ...
            if let (kuzu::Value::Int64(refno), kuzu::Value::String(name),
                    kuzu::Value::String(value_str), kuzu::Value::String(attr_type)) =
                (refno_val, name_val, value_val, type_val) {

                let refno_enum = RefnoEnum::from(RefU64(*refno as u64));
                let attr_map = attmaps.entry(refno_enum).or_insert_with(NamedAttrMap::default);
