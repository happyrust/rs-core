...
impl Circle2D {
    ...
    pub fn from_three_points(j: &Vec2, k: &Vec2, l: &Vec2, is_center: bool) -> Circle2D {
        let mut clock_wise = false;
        let (center, r) = if is_center {
            (*k, j.distance(*k))
        }else{
            let a = j.x * (k.y - l.y) -
                j.y * (k.x - l.x) +
        ...
        Circle2D { center, r: f32_round_3(r), clock_wise, angle}
    }
    ...
    //     Circle2D { center: Vec2::new(f32_round_3(x), f32_round_3(y)), r: f32_round_3(r), clock_wise: false, angle: 0.0 }
    // }
}