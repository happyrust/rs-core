...
impl SctnSolid {
    ...
    //is_btm 是否是底部的face
    fn cal_sann_face(&self, is_btm: bool, dir: Vec3, angle: f32, r1: f32, r2: f32) -> Option<Face>{
        return None;
        ...
        if let Ok(mut f) = try_attach_plane(&[wire]){
            if let Surface::Plane(plane) = f.get_surface() {
                if plane.normal().dot(extrude_dir.vector3()) < 0.0 {
                    f = f.inverse();
                }
            }
            return Some(f);
    ...
    fn cal_spro_face(&self, is_btm: bool, profile: &SProfileData) -> Option<Face>{
        ...
        if self.arc_path.is_some() {
            offset_pt = self.arc_path.unwrap().0;
