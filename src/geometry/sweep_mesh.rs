...
use crate::prim_geo::spine::{SweepPath3D, SegmentPath, Line3D, Arc3D};
use crate::prim_geo::sweep_solid::SweepSolid;
use crate::parsed_data::CateProfileParam;
use crate::shape::pdms_shape::PlantMesh;
use crate::mesh_precision::LodMeshSettings;
use glam::{Vec2, Vec3, DVec3, Quat, Mat3};
...
fn generate_line_sweep(
    ...
    let path_length = (line.end - line.start).length();
    ...
    // 添加端面（起始面）
    // 为端面单独生成顶点，确保它们都在同一平面上
    let start_cap_base = vertices.len() as u32;
    let profile_center = profile_points.iter().fold(Vec2::ZERO, |acc, &p| acc + p) / profile_points.len() as f32;
    ...
    let local_center = right * profile_center.x + up * profile_center.y;
    ...
    for i in 0..(n_profile - 1) {
        indices.extend_from_slice(&[
            start_center_idx,
            start_cap_base + i as u32,
            start_cap_base + (i + 1) as u32,
        ]);
    }
    if profile_points.len() > 3 {
        indices.extend_from_slice(&[
            start_center_idx,
            start_cap_base + (n_profile - 1) as u32,
            start_cap_base,
        ]);
    }
    ...
    for &profile_pt in profile_points.iter() {
        ...
        let vertex = line.end + local_3d + path_dir * z_offset;
        vertices.push(vertex);
        normals.push(end_normal);  // 端面顶点都使用端面法向
    }
    
    // 端面中心点
    let end_center_idx = vertices.len() as u32;
    let center_end_z_offset = if drne.is_some() {
        let normal_z = end_normal.dot(path_dir);
        let normal_y = end_normal.dot(up);
        if normal_z.abs() > 0.001 {
            profile_center.y * (normal_y / normal_z)
    ...
    let center_3d_end = line.end + local_center + path_dir * center_end_z_offset;
    vertices.push(center_3d_end);
    normals.push(end_normal);
    
    // 端面三角形索引（使用端面自己的顶点，注意缠绕方向）
    for i in 0..(n_profile - 1) {
        indices.extend_from_slice(&[
            end_center_idx,
            end_cap_base + (i + 1) as u32,
            end_cap_base + i as u32,
        ]);
    }
    if profile_points.len() > 3 {
        indices.extend_from_slice(&[
            end_center_idx,
            end_cap_base,
            end_cap_base + (n_profile - 1) as u32,
        ]);
    }
...
fn generate_arc_sweep(
    ...
    let (right, up) = {
        let ref_vec = if start_tangent.y.abs() < 0.9 { Vec3::Y } else { Vec3::X };
        let right = ref_vec.cross(start_tangent).normalize();
        let up = start_tangent.cross(right).normalize();
        (right, up)
    };
    
    let center_3d_start = arc_segment.point_at(0.0) + right * profile_center.x + up * profile_center.y;
    vertices.push(center_3d_start);
    ...
    let (right_end, up_end) = {
        let ref_vec = if end_tangent.y.abs() < 0.9 { Vec3::Y } else { Vec3::X };
        let right = ref_vec.cross(end_tangent).normalize();
        let up = end_tangent.cross(right).normalize();
        (right, up)
    };
    
    let center_3d_end = arc_segment.point_at(1.0) + right_end * profile_center.x + up_end * profile_center.y;
    vertices.push(center_3d_end);
...
fn generate_multi_segment_sweep(
    ...
    let (right, up) = {
        let ref_vec = if first_tangent.y.abs() < 0.9 { Vec3::Y } else { Vec3::X };
    ...
    let end_center_idx = vertices.len() as u32;
    let (right_end, up_end) = {
        let ref_vec = if last_tangent.y.abs() < 0.9 { Vec3::Y } else { Vec3::X };
        let right = ref_vec.cross(last_tangent).normalize();
        let up = last_tangent.cross(right).normalize();
        (right, up)
    };
    
    let center_3d_end = path_samples.last().unwrap().0 + right_end * profile_center.x + up_end * profile_center.y;
    vertices.push(center_3d_end);
...
pub fn generate_sweep_solid_mesh(
    ...
    profile_points = apply_profile_transform(
        &profile_points,
        sweep.plax,
        sweep.bangle,
        sweep.lmirror,
