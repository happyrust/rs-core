...
use crate::{connect_surdb, get_children_pes, get_pe, NamedAttrValue, query_ele_filter_deep_children, query_filter_deep_children, RefU64, SUL_DB, SurlValue};
...
impl MaterialTfHavcList {
    ...
    pub fn into_hashmap(self) -> HashMap<String, String> {
        ...
        map.entry("螺栓数量".to_string()).or_insert(self.bolt_qty.to_string());
        ...
        map.entry("螺母数量".to_string()).or_insert(self.nut_qty.to_string());
        map.entry("螺母数量_2".to_string()).or_insert(self.washer_qty.to_string());
...
pub async fn get_tf_hvac_material(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    ...
    for refno in refnos {
        ...
        if pe.noun == "SITE".to_string() {
            ...
            data.append(&mut result);
            // FLEX
            let tapes = refnos.iter().filter(|x| x.noun == "FLEX".to_string()).map(|x| x.refno).collect::<Vec<_>>();
            let mut result = get_tf_hvac_flex_data(db, tapes).await?;
            data.append(&mut result);
            // BEND
            let bends = refnos.iter().filter(|x| x.noun == "BEND".to_string()).map(|x| x.refno).collect::<Vec<_>>();
            let mut result = get_tf_hvac_bend_data(db, bends).await?;
            data.append(&mut result);
            // OFST
            let bends = refnos.iter().filter(|x| x.noun == "OFST".to_string()).map(|x| x.refno).collect::<Vec<_>>();
            let mut result = get_tf_hvac_ofst_data(db, bends).await?;
            data.append(&mut result);
            // TRNS
            let trnses = refnos.iter().filter(|x| x.noun == "TRNS".to_string()).map(|x| x.refno).collect::<Vec<_>>();
            let mut result = get_tf_hvac_trns_data(db, trnses).await?;
            data.append(&mut result);
            // BRCO
            let brcos = refnos.iter().filter(|x| x.noun == "BRCO".to_string()).map(|x| x.refno).collect::<Vec<_>>();
            let mut result = get_tf_hvac_brco_data(db, brcos).await?;
            data.append(&mut result);
            // THRE
            let brcos = refnos.iter().filter(|x| x.noun == "THRE".to_string()).map(|x| x.refno).collect::<Vec<_>>();
            let mut result = get_tf_hvac_thre_data(db, brcos).await?;
            data.append(&mut result);
            // CAP
            let caps = refnos.iter().filter(|x| x.noun == "CAP".to_string()).map(|x| x.refno).collect::<Vec<_>>();
            let mut result = get_tf_hvac_cap_data(db, caps).await?;
            data.append(&mut result);
            // STIF
            let caps = refnos.iter().filter(|x| x.noun == "STIF".to_string()).map(|x| x.refno).collect::<Vec<_>>();
...
/// 获取 通风 风管管段 strt 的数据
async fn get_tf_hvac_strt_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
...
/// 获取 通风 风管管段 tape 的数据
async fn get_tf_hvac_tape_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let mut data = Vec::new();
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
    let sql = format!("select
...
/// 获取 通风 风管管段 flex 的数据
async fn get_tf_hvac_flex_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let mut data = Vec::new();
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
...
/// 获取 通风 风管管段 bend 的数据
async fn get_tf_hvac_bend_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let mut data = Vec::new();
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
    let sql = format!("select
...
fn change_hvac_result_to_map(input: Vec<HashMap<String, NamedAttrValue>>) -> Vec<HashMap<String, String>> {
    let chinese_name_map = get_hvac_chinese_name_map();
    let mut result = Vec::new();
    for i in input {
        let mut map = HashMap::new();
        for (k, v) in i {
            let Some(chinese) = chinese_name_map.get(&k) else { continue; };
            map.entry(chinese.to_string()).or_insert(v.get_val_as_string());
        }
        result.push(map);
    }
    result
...
fn get_hvac_chinese_name_map() -> HashMap<String, String> {
    ...
    map.entry("description".to_string()).or_insert("描述".to_string());
    map.entry("seg_code".to_string()).or_insert("管段编号".to_string());
    map.entry("sub_code".to_string()).or_insert("子项号".to_string());
    map.entry("material".to_string()).or_insert("材质".to_string());
    map.entry("pressure".to_string()).or_insert("压力等级".to_string());
    map.entry("length".to_string()).or_insert("风管长度".to_string());
    map.entry("width".to_string()).or_insert("风管宽度".to_string());
    map.entry("height".to_string()).or_insert("风管高度".to_string());
    map.entry("wall_thk".to_string()).or_insert("风管壁厚".to_string());
    map.entry("duct_area".to_string()).or_insert("风管面积".to_string());
    map.entry("duct_weight".to_string()).or_insert("法兰重量".to_string());
    map.entry("stif_sctn".to_string()).or_insert("加强筋型材".to_string());
    map.entry("stif_len".to_string()).or_insert("加强筋长度".to_string());
    map.entry("stif_wei".to_string()).or_insert("加强筋重量".to_string());
    map.entry("fl_type".to_string()).or_insert("法兰规格".to_string());
    map.entry("fl_len".to_string()).or_insert("法兰长度".to_string());
    map.entry("fl_wei".to_string()).or_insert("法兰重量".to_string());
    map.entry("washer_type".to_string()).or_insert("垫圈类型".to_string());
    map.entry("washer_len".to_string()).or_insert("垫圈长度".to_string());
    map.entry("bolt_qty".to_string()).or_insert("螺栓数量".to_string());
    map.entry("other_type".to_string()).or_insert("其它材料类型".to_string());
    map.entry("other_qty".to_string()).or_insert("其它材料数量".to_string());
    map.entry("stud".to_string()).or_insert("螺杆".to_string());
    map.entry("nut_qty".to_string()).or_insert("螺母数量".to_string());
    map.entry("washer_qty".to_string()).or_insert("螺母数量_2".to_string());
    map.entry("room_no".to_string()).or_insert("所在房间号".to_string());
    map.entry("system".to_string()).or_insert("系统".to_string());

    map
}

/// 获取 通风 风管管段 ofst 的数据
async fn get_tf_hvac_ofst_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
    let sql = format!("select
    fn::refno(id) as id,
...
/// 获取 通风 风管管段 trns 的数据
async fn get_tf_hvac_trns_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
    let sql = format!("select
        fn::refno(id) as id,
...
/// 获取 通风 风管管段 brco 的数据
async fn get_tf_hvac_brco_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
    let sql = format!("select
      fn::refno(id) as id,
...
/// 获取 通风 风管管段 thre 的数据
async fn get_tf_hvac_thre_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
    let sql = format!("select
    fn::refno(id) as id,
...
/// 获取 通风 风管管段 cap 的数据
async fn get_tf_hvac_cap_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
    let sql = format!("select
    fn::refno(id) as id,
...
/// 获取 通风 风管管段 stif 的数据
async fn get_tf_hvac_stif_data(db: &Surreal<Any>, refnos: Vec<RefU64>) -> anyhow::Result<Vec<HashMap<String, String>>> {
    if refnos.is_empty() { return Ok(vec![]); };
    let refnos = refnos.into_iter().map(|refno| refno.to_pe_key()).collect::<Vec<_>>();
    let sql = format!("select
    fn::refno(id) as id,
