...
/// 缓存统计信息
static CACHE_STATS: Lazy<Mutex<CacheStats>> = 
    Lazy::new(|| Mutex::new(CacheStats::new()));
...
mod tests {
    ...
    async fn test_endatu_cache() {
        ...
        // 第一次查询，应该缓存未命中
        let result1: Result<Option<u32>, EndatuError> = get_cached_endatu_index(parent, refno).await;
        assert!(result1.is_ok());
        
        // 第二次查询，应该缓存命中
        let result2: Result<Option<u32>, EndatuError> = get_cached_endatu_index(parent, refno).await;
