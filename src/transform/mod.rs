...
#[cached(result = true)]
pub async fn get_local_transform(refno: RefnoEnum, parent_refno: RefnoEnum) -> anyhow::Result<Option<Transform>> {
...
pub async fn get_local_mat4(refno: RefnoEnum, parent_refno: RefnoEnum) -> anyhow::Result<Option<DMat4>> {
    ...
    if att.contains_key("ZDIS") {
        if cur_type == "ENDATU" {
            ...
            .await?
            {
                ...
                rotation = quat;
                return Ok(Some(DMat4::from_rotation_translation(rotation, translation)));
            }
        } else {
    ...
    if !pos_line.is_empty() {
        ...
        let ancestor_refnos = crate::query_filter_ancestors(parent_refno, &crate::consts::HAS_PLIN_TYPES).await?;
