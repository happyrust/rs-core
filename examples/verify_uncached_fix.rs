...
async fn get_poinsp_world_transform_uncached(poinsp_refno: RefnoEnum) -> Result<Option<DMat4>> {
    ...
    let spine_transform = calculate_spine_transform_at_distance_uncached(&spline_pts, distance_along_spine, spine_ydir)?;
    ...
    println!("  SPINE路径变换位置: {:?}", spine_transform.w_axis.truncate());
    ...
    for _depth in 0..10 {
        ...
        if owner == current_refno {
            if let Some(pos) = att.get_position() {
                accumulated_transform = DMat4::from_translation(pos.as_dvec3()) * accumulated_transform;
            }
            break;
        }
