...
async fn main() -> Result<()> {
    ...
    let test_cases: Vec<PdmsTestCase> = serde_json::from_reader(reader).expect("Failed to parse test cases");
    ...
    for case in &test_cases {
        ...
        if let Some(local_pos) = att.get_position() {
            ...
            if let Some(gensec_mat) = get_world_mat4(gensec_refno, false).await? {
                ...
                if diff.length() < 1.0 { // 1mm tolerance
                     ...
                     if pts.len() >= 2 {
                         ...
                         if let Some((expected_y, expected_z)) = parse_wori(&case.wori_str) {
                             ...
                             if y_dot > 0.9999 && z_dot > 0.9999 {
                                 ...
                                 println!("   Y Axis Match: {:.6} (Expected {:?} vs Got {:?})", y_dot, expected_y, y_axis);
                                 println!("   Z Axis Match: {:.6} (Expected {:?} vs Got {:?})", z_dot, expected_z, z_axis);
                                 ori_verified = true;
                             } else {
                                 println!("❌ WORI Failed:");
                                 println!("   Y Axis Dot: {:.6} (Expected {:?} vs Got {:?})", y_dot, expected_y, y_axis);
                                 println!("   Z Axis Dot: {:.6} (Expected {:?} vs Got {:?})", z_dot, expected_z, z_axis);
                             }
                         } else {
