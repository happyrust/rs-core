...
use aios_core::transform::{get_local_mat4, get_world_mat4};
...
async fn main() -> anyhow::Result<()> {
    ...
    if !posl.is_empty() {
        ...
        let plin_owners =
            aios_core::query_filter_ancestors(owner, &aios_core::consts::HAS_PLIN_TYPES).await?;
    ...
    println!(
        "  Parent {} ORI attr: {:?}",
        parent_refno_check,
        parent_att_check.get_str("ORI")
    );
    println!(
        "  Parent {} get_rotation(): {:?}",
        parent_refno_check,
        parent_att_check.get_rotation()
    );
    ...
    if let Some(mat) = my_local {
        ...
        println!(
            "  {} local POS: X {:.0}mm Y {:.0}mm Z {:.0}mm",
            refno, translation.x, translation.y, translation.z
        );
    } else {
    ...
    if cache_before.is_none()
        || cache_before
            .as_ref()
            .map(|c| c.world.is_none())
            .unwrap_or(true)
    {
        println!("  ✅ 确认缓存已清空");
    } else {
    ...
    let parent_local = get_local_mat4(parent_refno)
        .await?
