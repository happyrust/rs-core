...
async fn main() -> Result<()> {
    ...
    let fitt_refno: RefnoEnum = "25688/7960"
        .parse()
        .map_err(|e| anyhow::anyhow!("解析FITT参考号失败: {}", e))?;
    let parent_refno: RefnoEnum = "25688/7958"
        .parse()
    ...
    // 1. 使用新策略系统计算
    println!("\n📊 新策略系统实现:");
    if let Ok(Some(new_local)) =
        aios_core::transform::get_local_mat4(fitt_refno, parent_refno).await
    {
