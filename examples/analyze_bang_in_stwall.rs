...
use aios_core::{
    ...
    get_named_attmap, RefnoEnum,
    transform::{
        strategies::WallStrategy,
    },
};
use anyhow::Result;
...
fn simulate_stwall_with_bang(refno: RefnoEnum, test_bang: Option<f64>) -> (String, DMat4) {
    ...
    let parent_att = get_named_attmap(att.get_owner()).await.expect("Failed to get parent att");
    ...
    let result = strategy.get_local_transform().await.expect("Failed to get transform");
...
pub fn analyze_bang_effects() -> Result<()> {
    ...
    let refno = RefnoEnum::from_str("17496/202351")
        .map_err(|e| anyhow::anyhow!("Invalid refno: {}", e))?;
    ...
    if let (Some(dposs), Some(dpose)) = (att.get_dposs(), att.get_dpose()) {
        ...
        for (i, (desc, bang)) in test_cases.iter().enumerate() {
            ...
            println!("Y轴方向: ({:.3}, {:.3}, {:.3})", y_axis.x, y_axis.y, y_axis.z);
            
            // 计算Z轴旋转了多少度
            let z_angle = transform.w_axis.z.atan2(transform.w_axis.x) as f64 * 180.0 / std::f64::consts::PI;
