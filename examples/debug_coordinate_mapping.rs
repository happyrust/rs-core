...
async fn main() -> Result<()> {
    ...
    let result1 = calculate_with_mapping1(&spline_pts, poinsp_local_pos, spine_ydir, &gensec_world_mat).await?;
    println!("假设1 (Y=路径, X=横向, Z=垂直): 误差 {:.3}mm", result1.error);
    println!("  计算位置: {:?}", result1.position);
    
    // 假设2: X=沿路径距离, Y=横向, Z=垂直
    let result2 = calculate_with_mapping2(&spline_pts, poinsp_local_pos, spine_ydir, &gensec_world_mat).await?;
    println!("假设2 (X=路径, Y=横向, Z=垂直): 误差 {:.3}mm", result2.error);
    println!("  计算位置: {:?}", result2.position);
    
    // 假设3: Z=沿路径距离, X=横向, Y=垂直
    let result3 = calculate_with_mapping3(&spline_pts, poinsp_local_pos, spine_ydir, &gensec_world_mat).await?;
    println!("假设3 (Z=路径, X=横向, Y=垂直): 误差 {:.3}mm", result3.error);
    println!("  计算位置: {:?}", result3.position);
    
    // 5. 分析最佳假设
    let results = vec![result1, result2, result3];
    let best_result = results.iter().min_by(|a, b| a.error.partial_cmp(&b.error).unwrap());
