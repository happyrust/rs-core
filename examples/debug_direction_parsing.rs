...
async fn main() -> Result<()> {
    ...
    for case in test_cases {
        ...
        // 分析差异
        if let (Some(old), Some(new)) = (parse_pdms_direction_old(case), parse_pdms_direction_correct(case)) {
    ...
    if let Some(y_expected) = parse_pdms_direction_correct(y_desc) {
        ...
        println!("  仰角: {:.3}°", (y_expected.z.atan2(y_expected.y).to_degrees()));
    }
    ...
    if let Some(z_expected) = parse_pdms_direction_correct(z_desc) {
        ...
        let vertical_angle = (-z_expected.z).atan2((z_expected.x * z_expected.x + z_expected.y * z_expected.y).sqrt()).to_degrees();
