...
- **空结果子查询判断**（常见坑）：用子查询判断「是否存在某记录」时，无记录时子查询返回 `[]`，`[] = NONE` 为 **false**，会错误排除本应保留的行
  - ❌ 错误：`WHERE (SELECT ... LIMIT 1) = NONE`（无记录时 `[] = NONE` 为 false，全部被过滤）
  - ✅ 推荐：`WHERE (SELECT ... LIMIT 1) = []`（语义直接：结果是否为空）
  - ✅ 等价：`WHERE (SELECT ... LIMIT 1)[0] = NONE`（无记录时 `[][0]` 为 NONE）
