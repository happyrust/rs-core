...
**ID 格式**：`pe:⟨ref0_ref1⟩`，例如 `pe:⟨21491_10000⟩`（ref0=RefU64 高32位；ref1=低32位；ref0 不是 dbnum）

**关键字段**：
- `id`: 记录唯一标识符（RefnoEnum 类型）
- `refno`: RefU64 类型（完整参考号，可分解为 ref_0/ref_1；不等同 dbnum）
...
**注意**：
- 若只拿到 ref0（RefU64 高32位）而需 dbnum：须由 `DbMetaManager` 做 `ref0 -> dbnum` 映射；不可把 ref0 直接当 dbnum。
...
```rust
// RefnoEnum 转 PE key
let pe_key = refno.to_pe_key();  // 返回 "pe:⟨ref0_ref1⟩"（例如 "pe:⟨12345_67890⟩"）
...
**注意**：`pe.children` 为有序数组，其顺序即先后顺序；若需要按“原始先后”处理子节点，请直接使用该顺序，不要再 `ORDER BY`/额外排序。
...
```sql
-- 查询 BRAN 的管道实例（使用 ID Range；bran_refno 取值为 ref0_ref1，下划线连接）
