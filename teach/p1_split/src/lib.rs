...
/// Shared fixture loader (used by unit tests and wiring_sketch).
pub mod fixture_load {
    use super::SplitMember;
    use serde::Deserialize;
    use std::fs;
    ...
    pub struct FixtureCase {
        pub id: String,
        pub expect_allarr: Vec<Vec<String>>,
        pub members: Vec<SplitMember>,
    }

    pub fn load_fixtures() -> FixtureFile {
        let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../fixtures/p1-split-isolines.json");
        let text = fs::read_to_string(&path)
            .unwrap_or_else(|e| panic!("read {}: {e}", path.display()));
        serde_json::from_str(&text).expect("parse p1-split-isolines.json")
    }

    pub fn load_case(id: &str) -> FixtureCase {
        load_fixtures()
            .cases
            .into_iter()
            .find(|c| c.id == id)
...
mod tests {
    use super::*;
    use fixture_load::{load_case, load_fixtures};
