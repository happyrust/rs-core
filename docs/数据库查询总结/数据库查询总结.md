...
**⚠️ 性能提示**：层级查询不要在业务代码里手写这些语句，走封装（详见 [第 8 章 层级查询封装](#8-层级查询封装)）。
...
上面的裸语句适合在 CLI 里临时排查；业务代码里的**层级关系查询**（子节点、子孙节点、祖先节点）一律走封装：
...
```rust
// 使用 query_single_by_paths 遍历属性关系
// 这些是 PDMS 属性关系，不是层级关系，因此不能套用 collect_* 那套层级封装
...
**注意**：属性关系只能靠图遍历表达，层级封装（`collect_*`）只处理父子关系，不处理属性引用关系。

#### 3.4.2 层级关系遍历（走封装）

对于**层级关系查询**（子节点、子孙节点、祖先节点），用现成封装替代手写图遍历：
...
| 查询类型 | 手写 SurrealQL | 应改用的封装 |
|---------|-----------------|-----------|
| 子节点查询 | `<-pe_owner` 关系表 | `collect_children_filter_ids` |
| 子孙节点查询 | `@.{..}.children` 递归 | `collect_descendant_filter_ids` |
| 祖先节点查询 | `owner` 向上遍历 | `query_filter_ancestors` |
| 属性关系（GMRE/GSTR） | `->GMRE`, `->GSTR` | 无封装，直接图遍历 |
| 属性关系（LSTU/CATR） | `->LSTU->CATR` | 无封装，直接图遍历 |

**使用建议**：
1. **层级查询**：走 `collect_*` 系列封装
2. **属性关系**：用 `query_single_by_paths`
3. **混合查询**：先用 `collect_*` 拿到层级 refno 集合，再批量查属性

#### 3.4.3 层级关系查询改写示例

**不要这样写**：
```rust
// ❌ 通过 pe_owner 关系表手查子节点
// SELECT VALUE in FROM pe:⟨refno}⟩<-pe_owner;
...
层级遍历（子节点、子孙节点、祖先节点）统一走 `src/rs_surreal/graph.rs` 的封装函数，
它们内部已处理深度限制、noun 过滤和去重，不要在业务代码里手写 `pe_owner` 递归。

### 历史：TreeIndex 已下线

早期版本有一套基于 `indextree` 的 `.tree` 内存索引（`TreeIndex` / `TreeIndexQueryProvider`
/ `output/scene_tree/{dbnum}.tree` / `--gen-indextree`），号称比 SurrealDB 递归快百倍。
这套东西已于 2026-07 全部下线并从代码库删除，`indextree` 也已从依赖图移除。
现在：

- **层级关系** → 本章的 `collect_*` / `query_filter_ancestors` 封装（`pe_owner` 关系表）
- **一次 run 内反复遍历同一棵树** → `plant-model-gen` 的 `versioned_db/pe_owner_snapshot.rs`
  内存快照（按 run 失效，不是常驻缓存）
- **PE / 属性 / 实例** → 照旧走 SurrealDB

如果你在别处看到"优先使用 TreeIndex"，那是过期文档。
...
应用层
  ↓
query_provider::SurrealQueryProvider（或 QueryRouter）
...
| 查询类型 | 数据源 |
|---------|--------|
| 层级查询（子节点/子孙/祖先） | `pe_owner` 关系表 |
| PE 查询（get_pe） | `pe` 表 |
| 属性查询（get_named_attmap） | `named_attr` 表 |
| 实例查询（inst_relate） | `inst_relate` 表 |
...
1. **层级查询走封装**，不要手写 `pe_owner` 递归
2. **批量查询优于循环查询**（`collect_descendant_filter_ids` 支持一次传多个根）
...
- **多源路由**: `rs-core/src/query_provider/router.rs`
- **层级查询 trait**: `rs-core/src/tree_query.rs`（`TreeQuery` / `TreeQueryFilter` / `TreeQueryOptions`）
- **按 run 失效的层级快照**: `plant-model-gen/src/versioned_db/pe_owner_snapshot.rs`
