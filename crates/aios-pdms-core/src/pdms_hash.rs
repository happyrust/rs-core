...
mod tests {
    ...
    #[test]
    fn db1_hash_round_trips_pdms_name() {
        let hash = db1_hash("SCTN");
        assert_eq!(db1_hash_const("SCTN"), hash);
        assert_eq!(db1_hash_i32("SCTN") as u32, hash);
        assert_eq!(db1_dehash(hash), "SCTN");
    }

    #[test]
    fn uda_helpers_identify_uda_names_and_hashes() {
        assert!(is_uda_name(":UDA"));
        assert!(!is_uda_name("SCTN"));
        assert_eq!(get_uda_index(0x181FAD39), Some(1));
        assert!(is_uda(0x181FAD39));
    }
}
